...
def get_exp_list(log_dir, dataset_names):
    ...
    for folder in folders:
        name_parse = re.match(rf"(.+?)_({dataset_names_pattern})_FOLD(\d+)_finetune_(.*)", folder)
        if name_parse:
            exp_name = name_parse.group(1)
            dataset_name = name_parse.group(2) + "_" + name_parse.group(4)  # Combine dataset name and percentage
            fold = name_parse.group(3)
            # pct = name_parse.group(4)  # Capture the percentage part
